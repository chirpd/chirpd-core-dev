# Chirpd: The Church of Invocation, Respect, Prayer and Devotion 
...
1. [Download](https://github.com/chirpd/chirpd-core-platform/archive/master.zip) the The Chirpd Platform core
...
We make no assumptions about how you will use The Chirpd Platform. It is up to you to craft whatever solution is best for your use-case. We are doing our best to keep the core robust and controllable, so you can enable or disable different components.
...
We use the term `component` for a The Chirpd Platform object (in Symphony, objects are called *sections*). All The Chirpd Platform components have a template XSL file, which lives in the *workspace/themes/yourtheme* directory, with its component name.

We made an out of the box template for you to work from called [anchor](https://github.com/anchorchurch/anchor) in which we use the name "Anchor Church." Church Deloy will use whatever theme that you title `active`. So if you want to switch templates, simply go to the templates directory and change the folder name. The Chirpd Platform is designed to have all of the core functionalty in the parent project and let your theme template files only make the changes that are unique to your installation, but it isn't limiting in the least.

All of the components we used are free for you to use with The Chirpd Platform as it is a combination of open-source projects. Thank you open source community!
...
**We can't wait to see how you use The Chirpd Platform**
